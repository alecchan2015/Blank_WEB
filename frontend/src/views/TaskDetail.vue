<template>
  <div class="task-detail">
    <div class="page-header">
      <el-button text @click="$router.push('/dashboard')"><el-icon><ArrowLeft /></el-icon> 返回</el-button>
        ...
        <div v-if="agentFiles[agentType]?.length" class="file-section">
          ...
          <div class="file-list">
            <div v-for="file in agentFiles[agentType]" :key="file.id" class="file-item">
              ...
              <span class="file-credits">{{ file.credits }} 积分</span>
...
function initAgentStatuses() {
  orderedAgents.value.forEach((a, i) => {
    agentStatus.value[a] = i === 0 ? 'waiting' : 'waiting'
    ...
    if (data.type === 'agent_start') {
      ...
      if (data.files?.length) {
        agentFiles.value[data.agent] = data.files.map(f => ({
          id: f.id, type: f.type, name: f.name, credits: f.type === 'png' ? 15 : f.type === 'pdf' ? 10 : 5
        }))
      }
...
async function downloadFile(file) {
  ...
  try {
    ...
    await store.fetchMe()
    ElMessage.success(`下载成功，已扣除 ${file.credits} 积分`)
  } catch (e) {
...
.header-info h2 { font-size: 20px; color: #1a1a2e; }
...
.agent-result-block { background: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
.agent-result-header {
